...
pub fn generate_rsa_modulus(size: usize,
                            ctx: &mut BigNumberContext) -> IndyCryptoResult<(BigNumber, BigNumber, BigNumber)> {
    if size % 2 != 0 {
        return Err(err_msg(IndyCryptoErrorKind::InvalidParam(1), format!("Need an even number of bits, found {}", size)))
    }
