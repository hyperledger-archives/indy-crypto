...
pub fn get_pedersen_commitment(gen_1: &BigNumber, m: &BigNumber,
    ...
    let commitment = gen_1.mod_exp(m, modulus, Some(ctx))?.mod_mul(&gen_2.mod_exp(r, modulus, Some(ctx))?, modulus, Some(ctx))?;
    Ok(commitment)
}
...
pub fn get_generalized_pedersen_commitment(to_commit: Vec<(&BigNumber, &BigNumber)>,
    ...
    //let commitment = accumulated.mod_mul(&mod_exp(&gen_2, r, modulus, ctx)?, modulus, Some(ctx))?;
    Ok(accumulated)
}
