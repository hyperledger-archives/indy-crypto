...
impl ProofOfPossession {
    ...
    /// ```
    pub fn new(ver_key: &VerKey, sign_key: &SignKey) -> IndyCryptoResult<ProofOfPossession> {
    ...
    /// //TODO: Provide an example!
    /// ```
    pub fn from_bytes(bytes: &[u8]) -> IndyCryptoResult<ProofOfPossession> {
...
impl Signature {
    ...
    /// //TODO: Provide an example!
    /// ```
    pub fn from_bytes(bytes: &[u8]) -> IndyCryptoResult<Signature> {
        let point = PointG1::from_bytes(bytes)?;
        Ok(
            Signature {
                point,
...
impl MultiSignature {
   ...
   /// ```
    pub fn new(signatures: &[&Signature]) -> IndyCryptoResult<MultiSignature> {
    ...
    /// //TODO: Provide an example!
    /// ```
    pub fn from_bytes(bytes: &[u8]) -> IndyCryptoResult<MultiSignature> {
        let point = PointG1::from_bytes(bytes)?;
        Ok(
            MultiSignature {
                point: point,
...
impl Bls {
    ...
    /// ```
    pub fn sign(message: &[u8], sign_key: &SignKey) -> IndyCryptoResult<Signature> {
    ...
    /// assert!(valid);
    /// ```
    pub fn verify(signature: &Signature, message: &[u8], ver_key: &VerKey, gen: &Generator) -> IndyCryptoResult<bool> {
    ...
    /// assert!(valid);
    /// ```
    pub fn verify_proof_of_posession(pop: &ProofOfPossession, ver_key: &VerKey, gen: &Generator) -> IndyCryptoResult<bool> {
    ...
    /// assert!(valid)
    /// ```
    pub fn verify_multi_sig(multi_sig: &MultiSignature, message: &[u8], ver_keys: &[&VerKey], gen: &Generator) -> IndyCryptoResult<bool> {
    ...
    fn _gen_signature<T>(message: &[u8], sign_key: &SignKey, hasher: T) -> IndyCryptoResult<PointG1> where T: Digest {
        Bls::_hash(message, hasher)?.mul(&sign_key.group_order_element)
    }

    pub fn _verify_signature<T>(signature: &PointG1, message: &[u8], ver_key: &PointG2, gen: &Generator, hasher: T) -> IndyCryptoResult<bool> where T: Digest {
        let h = Bls::_hash(message, hasher)?;
        Ok(Pair::pair(&signature, &gen.point)?.eq(&Pair::pair(&h, &ver_key)?))
    }

    fn _hash<T>(message: &[u8], mut hasher: T) -> IndyCryptoResult<PointG1> where T: Digest {
