...
#[cfg(test)]
pub fn bn_rand(size: usize) -> IndyCryptoResult<BigNumber> {
...
pub fn _bn_rand(size: usize) -> IndyCryptoResult<BigNumber> {
...
#[cfg(test)]
pub fn bn_rand_range(_bn: &BigNumber) -> IndyCryptoResult<BigNumber> {
...
#[cfg(not(test))]
pub fn bn_rand_range(bn: &BigNumber) -> IndyCryptoResult<BigNumber> {
    _bn_rand_range(bn)
}

pub fn _bn_rand_range(bn: &BigNumber) -> IndyCryptoResult<BigNumber> {
...
pub fn encode_attribute(attribute: &str, byte_order: ByteOrder) -> IndyCryptoResult<BigNumber> {
...
#[cfg(test)]
pub fn generate_v_prime_prime() -> IndyCryptoResult<BigNumber> {
    if MockHelper::is_injected() {
...
pub fn _generate_v_prime_prime() -> IndyCryptoResult<BigNumber> {
...
pub fn _generate_prime_in_range(start: &BigNumber, end: &BigNumber) -> IndyCryptoResult<BigNumber> {
...
pub fn _generate_safe_prime(size: usize) -> IndyCryptoResult<BigNumber> {
...
#[cfg(test)]
pub fn gen_x(p: &BigNumber, q: &BigNumber) -> IndyCryptoResult<BigNumber> {
    if MockHelper::is_injected() {
...
pub fn _gen_x(p: &BigNumber, q: &BigNumber) -> IndyCryptoResult<BigNumber> {
...
pub fn _random_qr(n: &BigNumber) -> IndyCryptoResult<BigNumber> {
...
pub fn bitwise_or_big_int(a: &BigNumber, b: &BigNumber) -> IndyCryptoResult<BigNumber> {
...
pub fn get_mtilde(unrevealed_attrs: &HashSet<String>, mtilde: &mut HashMap<String, BigNumber>) -> IndyCryptoResult<()> {
...
pub fn calc_teq(p_pub_key: &CredentialPrimaryPublicKey,
                ...
                m2tilde: &BigNumber,
                unrevealed_attrs: &HashSet<String>) -> IndyCryptoResult<BigNumber> {
    ...
    for k in unrevealed_attrs.iter() {
        let cur_r = p_pub_key.r.get(k)
            .ok_or(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in pk.r", k)))?;
        let cur_m = m_tilde.get(k)
            .ok_or(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in m_tilde", k)))?;
...
pub fn calc_tge(p_pub_key: &CredentialPrimaryPublicKey,
                ...
                mj: &BigNumber,
                alpha: &BigNumber,
                t: &HashMap<String, BigNumber>) -> IndyCryptoResult<Vec<BigNumber>> {
    ...
    for i in 0..ITERATION {
        let cur_u = u.get(&i.to_string())
            .ok_or(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in u", i)))?;
        let cur_r = r.get(&i.to_string())
            .ok_or(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in r", i)))?;
    ...
    let delta = r.get("DELTA")
        .ok_or(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in r", "DELTA")))?;
    ...
    for i in 0..ITERATION {
        let cur_t = t.get(&i.to_string())
            .ok_or(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in t", i)))?;
        let cur_u = u.get(&i.to_string())
            .ok_or(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in u", i)))?;
...
pub fn four_squares(delta: i32) -> IndyCryptoResult<HashMap<String, BigNumber>> {
    trace!("Helpers::four_squares: >>> delta: {:?}", delta);

    if delta < 0 {
        return Err(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Cannot express a negative number as sum of four squares {} ", delta)));
    }
...
pub fn group_element_to_bignum(el: &GroupOrderElement) -> IndyCryptoResult<BigNumber> {
    Ok(BigNumber::from_bytes(&el.to_bytes()?)?)
}

pub fn bignum_to_group_element(num: &BigNumber) -> IndyCryptoResult<GroupOrderElement> {
...
pub fn create_tau_list_expected_values(r_pub_key: &CredentialRevocationPublicKey,
                                       ...
                                       proof_c: &NonRevocProofCList) -> IndyCryptoResult<NonRevocProofTauList> {
