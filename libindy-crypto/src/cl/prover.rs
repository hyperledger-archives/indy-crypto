...
impl Prover {
    ...
    pub fn blind_credential_secrets(credential_pub_key: &CredentialPublicKey,
                                    ...
                                    credential_nonce: &Nonce) -> IndyCryptoResult<(BlindedCredentialSecrets,
    ...
    pub fn process_credential_signature(credential_signature: &mut CredentialSignature,
                                        ...
                                        witness: Option<&Witness>) -> IndyCryptoResult<()> {
    ...
    pub fn check_credential_key_correctness_proof(pr_pub_key: &CredentialPrimaryPublicKey,
                                                  key_correctness_proof: &CredentialKeyCorrectnessProof) -> IndyCryptoResult<()> {
    ...
    fn _check_credential_key_correctness_proof(pr_pub_key: &CredentialPrimaryPublicKey,
        ...
        for r_key in pr_pub_key.r.keys() {
            if !correctness_names.contains(r_key) {
                ...
                if r_key != "master_secret" {
                    return Err(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in key_correctness_proof.xr_cap", r_key)));
                }
            }
        }
        for correctness_name in &correctness_names {
            if !pr_pub_key.r.contains_key(correctness_name.as_str()) {
                return Err(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Public key doesn't contains item for {} key in key_correctness_proof.xr_cap", correctness_name)));
            }
        }
        ...
        if !valid {
            return Err(err_msg(IndyCryptoErrorKind::InvalidStructure, "Invalid Credential key correctness proof"));
        }
    ...
    fn _generate_blinded_primary_credential_secrets_factors(p_pub_key: &CredentialPrimaryPublicKey,
                                                            credential_values: &CredentialValues) -> IndyCryptoResult<PrimaryBlindedCredentialSecretsFactors> {
        ...
        let u = hidden_attributes.iter().fold(
            ...
            |acc, attr| {
                let pk_r = p_pub_key.r.get(&attr.clone()).ok_or(
                    err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in pk.r", attr))
                )?;
    ...
    fn _generate_blinded_revocation_credential_secrets(r_pub_key: &CredentialRevocationPublicKey) -> IndyCryptoResult<RevocationBlindedCredentialSecretsFactors> {
    ...
    fn _new_blinded_credential_secrets_correctness_proof(p_pub_key: &CredentialPrimaryPublicKey,
                                                         ...
                                                         nonce: &BigNumber,
                                                         credential_values: &CredentialValues) -> IndyCryptoResult<BlindedCredentialSecretsCorrectnessProof> {
            ...
            match *cred_value {
                ...
                CredentialValue::Commitment { .. } => {
                    ...
                    let ca_value = blinded_primary_credential_secrets.committed_attributes
                        .get(attr)
                        .ok_or(
                            err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in primary_blinded_cred_secrets.committed_attributes", attr)))?;
        ...
        for (attr, m_tilde) in &m_tildes {
            let ca = credential_values.attrs_values.get(attr).ok_or(
                err_msg(IndyCryptoErrorKind::InvalidStructure, format!(
    ...
    fn _process_primary_credential(p_cred: &mut PrimaryCredentialSignature,
                                   v_prime: &BigNumber) -> IndyCryptoResult<()> {
    ...
    fn _process_non_revocation_credential(r_cred: &mut NonRevocationCredentialSignature,
                                          ...
                                          witness: &Witness) -> IndyCryptoResult<()> {
    ...
    fn _check_signature_correctness_proof(p_cred_sig: &PrimaryCredentialSignature,
        ...
        if !p_cred_sig.e.is_prime(Some(&mut ctx))? {
            return Err(err_msg(IndyCryptoErrorKind::InvalidStructure, "Invalid Signature correctness proof"));
        }
        ...
        if let Some((ref attr, _)) = cred_values.attrs_values
            ...
            return Err(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in public key", attr)));
        }
        ...
        if !q.eq(&expected_q) {
            return Err(err_msg(IndyCryptoErrorKind::InvalidStructure, "Invalid Signature correctness proof q != q'"));
        }
        ...
        if !valid {
            return Err(err_msg(IndyCryptoErrorKind::InvalidStructure, "Invalid Signature correctness proof c != c'"));
        }
    ...
    fn _test_witness_signature(r_cred: &NonRevocationCredentialSignature,
                               ...
                               rev_reg: &RevocationRegistry,
                               witness: &Witness,
                               r_cnxt_m2: &BigNumber) -> IndyCryptoResult<()> {
        ...
        if z_calc != rev_key_pub.z {
            return Err(err_msg(IndyCryptoErrorKind::InvalidStructure, "Issuer is sending incorrect data"));
        }
...
impl ProofBuilder {
    ...
    pub fn add_common_attribute(&mut self, attr_name: &str) -> IndyCryptoResult<()> {
    ...
    /// ```
    pub fn finalize(&self, nonce: &Nonce) -> IndyCryptoResult<Proof> {
    ...
    fn _check_add_sub_proof_request_params_consistency(
        ...
        if schema_attrs != cred_attrs {
            return Err(err_msg(IndyCryptoErrorKind::InvalidStructure, "Credential doesn't correspond to credential schema"));
        }
    ...
    fn _init_primary_proof(common_attributes: &HashMap<String, BigNumber>,
                           ...
                           sub_proof_request: &SubProofRequest,
                           m2_t: Option<BigNumber>) -> IndyCryptoResult<PrimaryInitProof> {
    ...
    fn _init_non_revocation_proof(r_cred: &NonRevocationCredentialSignature,
                                  rev_reg: &RevocationRegistry,
                                  cred_rev_pub_key: &CredentialRevocationPublicKey,
                                  witness: &Witness) -> IndyCryptoResult<NonRevocInitProof> {
    ...
    fn _init_eq_proof(common_attributes: &HashMap<String, BigNumber>,
                      ...
                      non_cred_schema_elems: &NonCredentialSchema,
                      sub_proof_request: &SubProofRequest,
                      m2_t: Option<BigNumber>) -> IndyCryptoResult<PrimaryEqualInitProof> {
    ...
    fn _init_ge_proof(p_pub_key: &CredentialPrimaryPublicKey,
                      ...
                      cred_values: &CredentialValues,
                      predicate: &Predicate) -> IndyCryptoResult<PrimaryPredicateGEInitProof> {
        ...
        let attr_value = cred_values.attrs_values.get(k.as_str())
            .ok_or(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in cred_values", k)))?
            .value()
            .to_dec()?
            .parse::<i32>()
            .map_err(|_| err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' has invalid format", k)))?;

        let delta: i32 = attr_value - value;

        if delta < 0 {
            return Err(err_msg(IndyCryptoErrorKind::InvalidStructure, "Predicate is not satisfied"));
        }
        ...
        for i in 0..ITERATION {
            let cur_u = u.get(&i.to_string())
                .ok_or(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in u1", i)))?;
        ...
        let mj = m_tilde.get(k.as_str())
            .ok_or(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in eq_proof.mtilde", k)))?;
    ...
    fn _finalize_eq_proof(init_proof: &PrimaryEqualInitProof,
                          ...
                          non_cred_schema_elems: &NonCredentialSchema,
                          cred_values: &CredentialValues,
                          sub_proof_request: &SubProofRequest) -> IndyCryptoResult<PrimaryEqualProof> {
        trace!(
        ...
        for k in unrevealed_attrs.iter() {
            let cur_mtilde = init_proof.m_tilde.get(k)
                .ok_or(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in init_proof.mtilde", k)))?;

            let cur_val = cred_values.attrs_values.get(k)
                .ok_or(err_msg(IndyCryptoErrorKind::InvalidStructure, format!("Value by key '{}' not found in attributes_values", k)))?;
        ...
        for attr in sub_proof_request.revealed_attrs.iter() {
            revealed_attrs_with_values.insert(
                ...
                cred_values.attrs_values
                    .get(attr)
                    .ok_or(err_msg(IndyCryptoErrorKind::InvalidStructure, "Encoded value not found"))?
                    .value()
                    .clone()?,
            );
        }
    ...
    fn _finalize_ge_proof(c_h: &BigNumber,
                          ...
                          eq_proof: &PrimaryEqualProof) -> IndyCryptoResult<PrimaryPredicateGEProof> {
    ...
    fn _finalize_primary_proof(init_proof: &PrimaryInitProof,
                               ...
                               non_cred_schema_elems: &NonCredentialSchema,
                               cred_values: &CredentialValues,
                               sub_proof_request: &SubProofRequest) -> IndyCryptoResult<PrimaryProof> {
        trace!(
    ...
    fn _gen_c_list_params(r_cred: &NonRevocationCredentialSignature) -> IndyCryptoResult<NonRevocProofXList> {
    ...
    fn _create_c_list_values(r_cred: &NonRevocationCredentialSignature,
                             ...
                             witness: &Witness) -> IndyCryptoResult<NonRevocProofCList> {
    ...
    fn _gen_tau_list_params() -> IndyCryptoResult<NonRevocProofXList> {
    ...
    fn _finalize_non_revocation_proof(init_proof: &NonRevocInitProof, c_h: &BigNumber) -> IndyCryptoResult<NonRevocProof> {
