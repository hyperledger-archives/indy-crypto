...
impl Prover {
    ...
    /// * `master_secret_blinding_nonce` - Nonce used for creation of blinded_credential_secrets_correctness_proof.
    ///
    /// # Example
    /// ```
    ...
    pub fn blind_master_secret(credential_pub_key: &CredentialPublicKey,
                               ...
                               master_secret_blinding_nonce: &Nonce) -> Result<(BlindedCredentialSecrets,
        ...
        let master_secret_blinding_factor = CredentialSecretsBlindingFactors {
            v_prime: blinded_primary_master_secret.v_prime,
            vr_prime: blinded_revocation_master_secret.map(|d| d.vr_prime)
        };

        trace!("Prover::blind_master_secret: <<< blinded_credential_secrets: {:?}, master_secret_blinding_factor: {:?}, blinded_credential_secrets_correctness_proof: {:?},",
               blinded_credential_secrets, master_secret_blinding_factor, blinded_credential_secrets_correctness_proof);

        Ok((blinded_credential_secrets, master_secret_blinding_factor, blinded_credential_secrets_correctness_proof))
    }
    ...
    pub fn process_credential_signature(credential_signature: &mut CredentialSignature,
        ...
        trace!("Prover::process_credential_signature: >>> credential_signature: {:?},signature_correctness_proof: {:?}, credential_secrets_blinding_factors: {:?}, \
        master_secret: {:?}, credential_pub_key: {:?}, nonce: {:?}, rev_key_pub: {:?}, rev_reg: {:?}, witness: {:?}", credential_signature,
               signature_correctness_proof, credential_secrets_blinding_factors, master_secret, credential_pub_key, nonce, rev_key_pub, rev_reg, witness);
    ...
    fn _generate_blinded_primary_master_secret(p_pub_key: &CredentialPrimaryPublicKey,
                                               master_secret: &MasterSecret) -> Result<PrimaryBlindedCredentialSecretsFactors, IndyCryptoError> {
        ...
        let primary_blinded_credential_secrets = PrimaryBlindedCredentialSecretsFactors { u, v_prime };

        trace!("Prover::_generate_blinded_primary_master_secret: <<< primary_blinded_credential_secrets: {:?}", primary_blinded_credential_secrets);

        Ok(primary_blinded_credential_secrets)
    }

    fn _generate_blinded_revocation_master_secret(r_pub_key: &CredentialRevocationPublicKey) -> Result<RevocationBlindedCredentialSecretsFactors, IndyCryptoError> {
        ...
        trace!("Prover::_generate_blinded_revocation_master_secret: <<< revocation_blinded_credential_secrets: {:?}", revocation_blinded_credential_secrets);
    ...
    fn _new_blinded_credential_secrets_correctness_proof(p_pub_key: &CredentialPrimaryPublicKey,
                                                    blinded_credential_secrets: &PrimaryBlindedCredentialSecretsFactors,
                                                    nonce: &BigNumber,
                                                    master_secret: &MasterSecret) -> Result<BlindedCredentialSecretsCorrectnessProof, IndyCryptoError> {
        trace!("Prover::_new_blinded_credential_secrets_correctness_proof: >>> p_pub_key: {:?}, blinded_credential_secrets: {:?}, nonce: {:?}, master_secret: {:?}",
               blinded_credential_secrets, nonce, p_pub_key, master_secret);
        ...
        let mut values: Vec<u8> = Vec::new();
        values.extend_from_slice(&blinded_credential_secrets.u.to_bytes()?);
        ...
        let v_dash_cap =
            c.mul(&blinded_credential_secrets.v_prime, Some(&mut ctx))?
                .add(&v_dash_tilde)?;
        ...
        let blinded_primary_master_secret_correctness_proof = BlindedCredentialSecretsCorrectnessProof { c, v_dash_cap, ms_cap };
...
mod tests {
    ...
    fn generate_blinded_primary_master_secret_works() {
        ...
        assert_eq!(blinded_primary_master_secret, mocks::primary_blinded_credential_secrets_data());
    }
    ...
    fn generate_blinded_credential_secrets_works() {
        ...
        assert_eq!(blinded_credential_secrets.u, mocks::primary_blinded_credential_secrets_data().u);
        assert_eq!(credential_secrets_blinding_factors.v_prime, mocks::primary_blinded_credential_secrets_data().v_prime);
    ...
    fn process_primary_credential_works() {
        ...
        let v_prime = mocks::primary_blinded_credential_secrets_data().v_prime;
    ...
    fn process_credential_works() {
        ...
        Prover::process_credential_signature(&mut credential_signature, &credential_values, &signature_correctness_proof,
                                             &credential_secrets_blinding_factors, &master_secret, &pk, &nonce, None, None, None).unwrap();
...
pub mod mocks {
    ...
    pub fn blinded_credential_secrets() -> BlindedCredentialSecrets {
        BlindedCredentialSecrets {
            u: primary_blinded_credential_secrets_data().u,
    ...
    pub fn credential_secrets_blinding_factors() -> CredentialSecretsBlindingFactors {
        CredentialSecretsBlindingFactors {
            v_prime: primary_blinded_credential_secrets_data().v_prime,
            vr_prime: Some(GroupOrderElement::new().unwrap())
        }
    }

    pub fn primary_blinded_credential_secrets_data() -> PrimaryBlindedCredentialSecretsFactors {
