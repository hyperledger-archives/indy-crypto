...
impl Issuer {
    ...
    ///     Prover::blind_master_secret(&cred_pub_key, &cred_key_correctness_proof, &master_secret, &credential_nonce).unwrap();
    ///
    /// let mut credential_values_builder = Issuer::new_credential_values_builder().unwrap();
    /// credential_values_builder.add_dec_known("name", "1139481716457488690172217916278103335").unwrap();
    ...
    fn _new_credential_primary_keys(credential_schema: &CredentialSchema) -> Result<(CredentialPrimaryPublicKey,
        ...
        let mut p = p_safe.decrement()?;
        p.div_word(2)?;

        let mut q = q_safe.decrement()?;
        q.div_word(2)?;
    ...
    fn _check_blinded_credential_secrets_correctness_proof(blinded_cred_secrets: &BlindedCredentialSecrets,
        ...
        let u_cap =
            blinded_cred_secrets.u
                ...
                .mod_mul(
                    &cred_pr_pub_key.rms.mod_exp(&blinded_cred_secrets_correctness_proof.ms_cap, &cred_pr_pub_key.n, Some(&mut ctx))?,
                    &cred_pr_pub_key.n,
                    Some(&mut ctx)
                )?;
...
mod tests {
    ...
    fn sign_primary_credential_works() {
        ...
        let (credential_signature, q) = Issuer::_sign_primary_credential(&pub_key, &secret_key, &context_attribute, &credential_values, &v, &BlindedCredentialSecrets { u: u, ur: None }, &e).unwrap();
    ...
    fn sign_credential_signature_works() {
        ...
        let blinded_credential_secrets_nonce = new_nonce().unwrap();
